...
Vários usuários ao mesmo tempo & A API aceita as chamadas em paralelo. A espera surge só na geração local em CPU (padrão: uma por vez). Provedor externo não cria essa fila & O mesmo no cluster: não é limite de infraestrutura do serviço. No Ollama a CPU redige uma geração por vez; no provedor, o paralelismo é do serviço \\
\bottomrule
\end{tabularx}
\end{table}
...
Várias pessoas podem usar a ferramenta ao mesmo tempo. Isso \textbf{não} é uma limitação de infraestrutura: a API aceita as chamadas em paralelo, a rede e o índice não serializam a consulta, e o backend não recusa o segundo usuário. O que impede duas redações locais ao mesmo tempo é a \textbf{CPU ocupada pelo modelo}. Uma geração do 7B em CPU usa o processador até terminar; a segunda espera essa geração, não um recurso que falte no cluster.

\textbf{Ollama em CPU.} A variável \texttt{RAG\_VAGAS\_GERACAO\_LOCAL} só organiza essa espera. O padrão do piloto é \textbf{1}: a segunda pessoa é aceita e fica na vez, no mesmo processo, até a primeira terminar. O código já admite um inteiro maior (teto interno 8), e o processo Ollama pode aceitar mais de uma geração (\texttt{OLLAMA\_NUM\_PARALLEL}). Subir a vaga não remove o gargalo: as gerações extras disputam a mesma CPU e a mesma RAM do modelo. Réplicas ou GPU fariam gerações de verdade em paralelo, e isso sim depende de capacidade no Rancher. Não é o caso deste piloto, e não é o que produz a fila de hoje.

\textbf{Provedor externo (OpenAI ou DeepSeek).} Por esse caminho \textbf{não há gargalo de multiusuário}. As chamadas não entram na fila local. Várias pessoas geram ao mesmo tempo, e o processamento paralelo é por conta do serviço, até o limite de taxa e de crédito da conta. A busca nos documentos permanece no perímetro; só o pedido de redação, com o trecho de contexto, sai para a internet. No perfil de produção fechado com a Agência esse caminho permanece desligado (\texttt{RAG\_LLM\_CLOUD\_FALLBACK=false}). Desligá-lo é uma escolha de perímetro, não uma falta de capacidade do backend.
...
\begin{itemize}
  ...
  \item Várias pessoas podem consultar ao mesmo tempo. A espera no Ollama local não é falta de infraestrutura: a API aceita as chamadas, e a CPU redige uma geração por vez (\texttt{RAG\_VAGAS\_GERACAO\_LOCAL}). Pelo provedor externo esse gargalo não existe; o paralelismo é do serviço.
\end{itemize}
...
O atendimento simultâneo não se confunde com essa limitação de infraestrutura. O backend já recebe várias pessoas ao mesmo tempo. No Ollama, a fila local (padrão: uma vaga) existe porque a geração ocupa a CPU, não porque o serviço ou o cluster recusem a segunda chamada. Em OpenAI ou DeepSeek esse gargalo não aparece: o processamento paralelo fica por conta do provedor. O perfil de produção da Agência segue no modelo local, e por isso a fila permanece.
...
\noindent\textit{Documento vivo: atualizar quando mudarem o perfil de infraestrutura ANTT, o hardware usado na simulação do deploy ou o teto de modelo local suportado. Última revisão: setembro de 2026 (premissa: desenvolvimento como simulação do perfil de operação; gargalos restritos a esse perfil; espera multiusuário só na geração local em CPU, inexistente no provedor externo).}
